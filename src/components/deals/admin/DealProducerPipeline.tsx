...
import { Loader2, CheckCircle, XCircle, Play, Pause, Download, ArrowRight, Music } from "lucide-react";
...
export default function DealProducerPipeline({ onNavigateToTracks }: Props) {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          Producer applications flow into the deal pipeline once approved.
        </p>
      </div>
      ...
      {filtered.length === 0 ? (
        <p className="text-muted-foreground text-center py-8">No {statusFilter === "all" ? "" : statusFilter} applications.</p>
      ) : (
                ...
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-3">
                  <div className="space-y-1 flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h3 className="font-semibold">{app.legal_name}</h3>
                      {app.stage_name && <span className="text-muted-foreground text-sm">({app.stage_name})</span>}
                      <Badge variant={statusColor(app.status)} className="text-xs">{app.status}</Badge>
                    </div>
                    <p className="text-sm text-muted-foreground">{app.email}</p>
                    <div className="flex gap-3 text-sm flex-wrap">
                      {app.genre && <span><span className="font-medium">Genre:</span> {app.genre}</span>}
                      {app.location && <span><span className="font-medium">Location:</span> {app.location}</span>}
                    </div>
                    {app.bio && <p className="text-sm text-muted-foreground mt-1 line-clamp-2">{app.bio}</p>}
                    <div className="flex gap-3 mt-1 flex-wrap">
                      {app.tiktok_url && <a href={app.tiktok_url} target="_blank" rel="noreferrer" className="text-xs text-primary underline">TikTok</a>}
                      {app.instagram_url && <a href={app.instagram_url} target="_blank" rel="noreferrer" className="text-xs text-primary underline">Instagram</a>}
                      {app.spotify_url && <a href={app.spotify_url} target="_blank" rel="noreferrer" className="text-xs text-primary underline">Spotify</a>}
                      {app.other_social_url && <a href={app.other_social_url} target="_blank" rel="noreferrer" className="text-xs text-primary underline">Other</a>}
                      {app.soundcloud_url && <a href={app.soundcloud_url} target="_blank" rel="noreferrer" className="text-xs text-primary underline">SoundCloud</a>}
                      ...
                      {app.website_url && <a href={app.website_url} target="_blank" rel="noreferrer" className="text-xs text-primary underline">Website</a>}
                    </div>
                    {app.demo_signed_url && (
                      <DemoPlayer url={app.demo_signed_url} name={`${app.legal_name}-demo`} />
                    )}
                    {app.rejection_reason && (
                      <p className="text-sm text-destructive mt-1">Rejection: {app.rejection_reason}</p>
                    )}
                  ...
                  <div className="flex gap-2 shrink-0 items-start">
                    ...
                    {app.status === "approved" && onNavigateToTracks && (
                      <Button size="sm" variant="outline" onClick={onNavigateToTracks}>
                        <Music className="h-4 w-4 mr-1" /> View Tracks <ArrowRight className="h-3 w-3 ml-1" />
                      </Button>
                    )}
                  </div>
                </div>
