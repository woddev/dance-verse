...
export default function ProducerEarnings() {
  ...
  useEffect(() => {
    Promise.all([api.getEarnings(), api.getPayouts()])
    ...
    <ProducerLayout>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        <Card>
          ...
          <CardContent>
            {loading ? <Skeleton className="h-8 w-24" /> : <div className="text-2xl font-bold">${totalEarned.toLocaleString("en-US", { minimumFractionDigits: 2 })}</div>}
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
          ...
          <CardContent>
            {loading ? <Skeleton className="h-8 w-24" /> : <div className="text-2xl font-bold">${pendingEarnings.toLocaleString("en-US", { minimumFractionDigits: 2 })}</div>}
          </CardContent>
        </Card>
      </div>
                ...
                <TableBody>
                  {earnings.map((e) => (
                    <TableRow key={e.id}>
                      ...
                      <TableCell className="text-right">${Number(e.gross_revenue).toFixed(2)}</TableCell>
                      <TableCell className="text-right">${Number(e.platform_fee).toFixed(2)}</TableCell>
                      <TableCell className="text-right">${Number(e.net_revenue).toFixed(2)}</TableCell>
                      <TableCell className="text-right font-medium">${Number(e.producer_amount).toFixed(2)}</TableCell>
